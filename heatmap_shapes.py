...
def generate_country_state_map(df, output_file="docs/country_state_map.png"):
    print("Generating country and state map...")
    ...
    visited_states = set()
    for res in results:
        visited_countries.add(res.get("cc"))
        if res.get("cc") == "US":
            visited_states.add(res.get("admin1"))

    incorrect_states = {
        "Washington", "Kentucky", "Michigan", "Nebraska", "Colorado",
        "New Mexico", "Nevada", "Maine", "Oklahoma", "Arkansas", "Kansas"
    }
    incorrect_countries = ["Zambia", "Greenland", "Belgium"]

    world = gpd.read_file("shapefiles/ne_110m_admin_0_countries.shp")
    us_states = gpd.read_file("shapefiles/cb_2021_us_state_20m.shp")
    world["highlight"] = world["NAME"].isin(incorrect_countries)

    iso3_visited = {
        pycountry.countries.get(alpha_2=cc).alpha_3
        for cc in visited_countries
        if pycountry.countries.get(alpha_2=cc)
    }
    world["visited"] = world["ADM0_A3"].isin(iso3_visited)

    us_states["visited"] = us_states["NAME"].isin(visited_states)
    us_states["highlight"] = us_states["NAME"].isin(incorrect_states)


    base = world.plot(color="lightgray", edgecolor="white", figsize=(14, 9), zorder=1)

    world[world["visited"] & ~world["highlight"]].plot(ax=base, color="lightgreen", edgecolor="black", zorder=2)
    world[world["highlight"]].plot(ax=base, color="gold", edgecolor="black", zorder=3)

    us_states[us_states["visited"] & ~us_states["highlight"]].plot(ax=base, color="lightgreen", edgecolor="black", zorder=4)
    us_states[us_states["highlight"]].plot(ax=base, color="gold", edgecolor="black", zorder=5)

    plt.title("Visited Countries and US States (Incorrect States in Gold)", fontsize=16)
    plt.axis("off")
    legend_patches = [
    mpatches.Patch(color="lightgreen", label="Visited Locations"),
    mpatches.Patch(color="gold", label="Incorrect Locations"),]
    plt.legend(handles=legend_patches, loc="upper right")
    os.makedirs("docs", exist_ok=True)
    plt.savefig(output_file)
    print(f"Saved country and state map to {output_file}")
